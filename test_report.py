...
def load_selenium_results():
    ...
    try:
        with open('result_test_selenium.json', 'r') as f:
            data = json.load(f)
            print(f"✅ Fichier result_test_selenium.json chargé ({len(data)} tests Selenium)")
            return data
    except FileNotFoundError:
        print("⚠️  result_test_selenium.json non trouvé (tests Selenium non disponibles)")
        return {}
    except json.JSONDecodeError as e:
...
def main():
    ...
    for test_id in sorted(tests.keys()):
        ...
        status, icon = get_test_status(test_id, test_type, django_results, selenium_results)
    ...
    if stats['total'] > 0:
        ...
        if selenium_total > 0:
            ...
            selenium_failed_pct = (stats['selenium_failed'] / selenium_total) * 100
            selenium_not_found_pct = (stats['selenium_not_found'] / selenium_total) * 100
            
            print(f"Tests Selenium exécutés: {selenium_total}")
            print(f"  ✅ Selenium passés: {stats['selenium_passed']} ({selenium_passed_pct:.1f}%)")
            print(f"  ❌ Selenium échoués: {stats['selenium_failed']} ({selenium_failed_pct:.1f}%)")
            print(f"  ❓ Selenium non trouvés: {stats['selenium_not_found']} ({selenium_not_found_pct:.1f}%)")
        else:
        ...
        for test_id, test_info in tests.items():
            ...
            if test_type in type_counts:
                ...
                status, _ = get_test_status(test_id, test_type, django_results, selenium_results)
                if status == "✔Passed":
    ...
    selenium_tests = [t for t in tests.items() if t[1].get('type') == 'auto-selenium']
    
    if selenium_tests:
        print(f"✅ {len(selenium_tests)} test(s) 'auto-selenium' détecté(s) dans test_list.yaml:")
        for test_id, test_info in selenium_tests:
            print(f"   - {test_id}: {test_info.get('description', 'Pas de description')}")
        ...
        for test_id, _ in selenium_tests:
            ...
            if result:
                ...
                print(f"   - {test_id}: ❌ Aucun résultat dans result_test_selenium.json")
    else:
        print("⚠️  Aucun test 'auto-selenium' trouvé dans test_list.yaml")
        print("   Assurez-vous d'avoir ajouté des tests avec type: 'auto-selenium'")
